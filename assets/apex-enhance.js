...
(function () {
 ...
 var WA_SVG =
   ...
   '<path d="M16.004 0C7.164 0 0 7.163 0 16c0 2.82.738 5.566 2.14 7.985L0 32l8.223-2.148A15.9 15.9 0 0 0 16.004 32C24.844 32 32 24.837 32 16S24.844 0 16.004 0zm0 29.278a13.23 13.23 0 0 1-6.75-1.849l-.484-.288-4.878 1.274 1.302-4.755-.315-.488a13.24 13.24 0 0 1-2.03-7.172c0-7.32 5.955-13.278 13.155-13.278 7.2 0 13.156 5.957 13.156 13.278 0 7.32-5.956 13.278-13.156 13.278zm7.223-9.928c-.396-.198-2.34-1.155-2.703-1.287-.363-.132-.627-.198-.891.198-.264.396-1.02 1.287-1.251 1.55-.231.265-.462.298-.858.1-.396-.198-1.671-.615-3.183-1.964-1.177-1.05-1.972-2.347-2.203-2.744-.231-.396-.024-.61.174-.807.178-.178.396-.463.594-.694.198-.231.264-.396.396-.66.132-.264.066-.495-.033-.693-.099-.198-.891-2.148-1.221-2.94-.32-.771-.646-.667-.891-.679a17.16 17.16 0 0 0-.759-.014c-.264 0-.693.099-1.056.495-.363.396-1.386 1.353-1.386 3.3s1.419 3.828 1.617 4.093c.198.264 2.79 4.26 6.76 5.973.945.408 1.681.652 2.256.834.948.302 1.81.259 2.492.157.76-.113 2.34-.957 2.67-1.88.33-.924.33-1.716.231-1.881-.099-.165-.363-.264-.759-.462z"></path></svg>';
...
".apex-rf-msg.ok{color:#1ea672;}",
 ...
 /* ------------------------------------------------------------------ reviews */
 // React still owns and periodically re-renders #testimonios (e.g. on
 // language switch). Never overwrite its innerHTML — that destroys nodes
 // React still holds references to, and on its next render it re-inserts
 // its own original content on top of ours (the overlapping-text bug).
 // Instead: hide #testimonios and keep our own reviews block in a single
 // dedicated wrapper, placed right before #servicios so it shows near the
 // top of the page, that only this script ever touches.
 function hideOriginalTestimonios() {
   var sec = document.getElementById("testimonios");
   if (sec && sec.id !== "apex-reviews-top" && sec.style.display !== "none") {
 ...
 function injectReviews() {
   ...
   if (!servicios) return;
   hideOriginalTestimonios();

   var wrap = document.getElementById("apex-reviews-top");
   var firstBuild = !wrap;
   if (firstBuild) {
     ensureStyle();
     wrap = document.createElement("section");
     wrap.id = "apex-reviews-top";
     servicios.parentNode.insertBefore(wrap, servicios);
   } else if (wrap.getAttribute("data-lang") === getLang()) {
     return; // already built for the current language — don't refetch/rebuild on unrelated DOM mutations
   }
   wrap.setAttribute("data-lang", getLang());
 ...
 // as a reliable fallback/seed so genuine customer feedback shows on the site
 // even without a live Places API connection. Merged with any live Google
 // API results (if configured) and site-submitted reviews.
 var GOOGLE_SEED_REVIEWS = [
   ...
   { author: "Shehryar Nadeem", rating: 5, text: "I had an excellent experience with Apex Detailing. They did an outstanding job detailing both the interior and exterior of my car, and the results were amazing. My car looks and feels brand new again! The team was professional, hardworking," },
 ...
 function renderReviewCards(grid, siteItems, googleData) {
   ...
   grid.innerHTML = cardsHtml || '<div class="apex-rev-empty">' + esc(t("Be the first to leave a review!")) + "</div>";
 ...
 function loadApprovedReviews() {
   ...
   // Show real review content immediately — no network wait. GOOGLE_SEED_REVIEWS
   // is already in memory, so there's no reason to make people stare at a
   // "Loading..." placeholder while we wait on the network.
   renderReviewCards(grid, [], {});

   // Then quietly fetch site-submitted reviews and live Google data in the
   // background and merge them in once available, without blocking display.
   fetchWithTimeout("/.netlify/functions/list-reviews-public", 7000)
     .then(function (r) { return r.json(); })
     .catch(function () { return []; })
     .then(function (siteResult) {
       var siteItems = (siteResult || []).filter(function (r) { return !r.static; });
       fetchWithTimeout("/.netlify/functions/google-reviews", 7000)
         .then(function (r) { return r.json(); })
         .catch(function () { return {}; })
         .then(function (googleData) {
           var g = document.getElementById("apex-rev-grid");
           if (!g) return;
           renderReviewCards(g, siteItems, googleData || {});
         });
     });
 }
