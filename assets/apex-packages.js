...
(function () {
   ...
   var PACKAGES = [
     ...
     },
     {
             ...
             includesBanner: "Everything in Exterior Detail, plus:",
     },
     {
             id: "complete",
             ...
             includesBanner: "Everything in Exterior & Interior, plus:",
     },
     {
             id: "premium",
             name: "Premium Detail",
             ...
             includesBanner: "Everything in Complete Detail, plus:",
     },
       ];
   ...
   function cardHTML(pkg) {
      ...
      return (
              ...
              (pkg.noCustomize
                       ? ""
                       : '<button class="edit-btn" data-act="edit" type="button">✏ Customize</button>') +
              "</div>" +
              ...
              collapsed +
              edit +
              '<div class="pkg-total-row"><div><p class="total-label">Estimated Total</p>' +
              '<p class="total-note">Starting price</p></div>' +
   ...
   function fullHTML() {
      ...
      return (
              ...
              "<p>Choose a package — or tap Customize to tailor it to your needs.</p>" +
              '<p class="disclaimer">⚠ All prices are initial estimates. The final price may vary based on the size, model, and cleanliness of the vehicle.</p>' +
              "</div>" +
              '<div class="packages-grid">' +
              cards +
              "</div>" +
              '<div class="extras-section"><h2>Additional <span>Services</span></h2>' +
              '<p class="section-subtitle">Enhance any package with these additional services.</p>' +
              '<div class="addons-grid">' +
              addons +
              "</div></div>" +
              '<div class="membership-section"><h2>Apex <span>Membership</span></h2>' +
              '<p class="section-subtitle">The best value for frequent customers.</p>' +
              '<div class="membership-card"><div class="membership-top">' +
              '<div><p class="membership-title">Apex Monthly Shine</p>' +
              '<p class="membership-subtitle">Everything you need, every month.</p></div>' +
              '<div><p class="membership-price-label">Per month</p>' +
              '<p class="membership-price">$99</p></div></div>' +
              '<hr style="border:none;border-top:1px solid #1e2a3a;">' +
              '<ul class="membership-features"><li>2 exterior details per month</li>' +
              "<li>10% discount on all premium services</li>" +
              "<li>Priority booking</li></ul>" +
   ...
   function renderCard(pkg) {
      ...
      if (!pkg.noCustomize) {
              ...
              eb.textContent = isEdit ? "✓ Done" : "✏ Customize";
      }
      ...
      if (editView) {
              ...
              editView.querySelectorAll(".service-item").forEach(function (el, li) {
                        ...
                        btn.title = isRemoved ? "Add" : "Remove";
                        btn.setAttribute("aria-label", isRemoved ? "Add" : "Remove");
      ...
      card.querySelector(".total-amount").textContent = "$" + (allGone ? 0 : total);
         card.querySelector(".total-note").textContent = allGone
           ? "No services selected"
                 : anyRemoved
           ? count + " service(s) removed · adjusted total"
                 : "Starting price";
